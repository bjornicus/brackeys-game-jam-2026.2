...
- [x] Trigger once on left Shift, right Shift, or middle-button `just_pressed`. Coalesce simultaneous inputs so one frame produces one cast.
- [x] Spawn one short-lived shockwave effect represented by several concentric circles. Expand them quickly from the player to exactly `CombatConfig.shockwave_radius`, fade them, then despawn.
- [x] Apply stun once per cast to all living enemies whose hitboxes are within the configured maximum radius. Use a circle-vs-AABB helper so range corresponds to hitboxes, not only transform origins.
- [x] Adapt `../bevy/examples/camera/2d_screen_shake.rs`: restore the unshaken transform in `PreUpdate`, track the player normally in `Update`, and apply trauma/noise in `PostUpdate` before `TransformSystems::Propagate`.
- [x] Keep cursor-to-world conversion and gameplay logic based on the restored/unshaken camera transform.
- [x] Put shake strength/decay/exponent/max rotation/max translation/noise speed in a tunable camera-shake config. The stun cast adds a configured amount of trauma.
- [x] Ensure pausing cannot leave the camera permanently offset: restoration must still occur when gameplay update systems are paused.
- [x] Stun does not damage enemies and may be cast while enemies are already stunned (resetting duration per Step 8 policy).
