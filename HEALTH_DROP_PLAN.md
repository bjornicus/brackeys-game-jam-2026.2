...
- [x] Add compact headless integration tests for partial heal, capped heal, full-health non-consumption, one-per-frame selection, exact boundary, modal priority, death guard, HUD refresh, Continue cleanup/determinism, and New Game seed refresh.
...
- `Step 2 — 2026-08-29`: Added gameplay-only automatic health-drop collection after terminal/progression-pickup triggers. Injured living players heal by the centralized amount, capped at maximum, and consume one deterministic overlapping drop; full-health and dying/zero-health players leave drops available. Added headless partial/capped, full/death guard, exact-boundary, one-per-frame, terminal/progression priority, same-frame HUD, Continue cleanup/seed preservation, and New Game seed-refresh coverage. README documents health-drop tuning. Files changed: `src/game.rs`, `README.md`, `HEALTH_DROP_PLAN.md`. Final `cargo fmt --check`, targeted health-drop tests, `cargo test`, and `cargo check --all-targets` passed. Manual drop walkthrough not run.
